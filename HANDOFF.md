...
>
> **Open decision (#3, still open):** per-set feedback strategy — AI on notable sets only / full AI every set / smart rules only. Step 0 is now resolved (AI confirmed working, and the failures were the key-read bug, NOT quota), so this can be picked on the merits. Note per-set reactions are currently rule-based-only ([`reactionForSet`](./src/features/coach/logic.ts)) and never call AI. `npm run build` green, `tsc` clean. Frontend v0.14.0 pushed (commits `c470bb8`); all 4 edge functions fixed + redeployed (`cb52f8d`, `91fe86f`).

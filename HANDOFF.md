...
> **🤖 Real AI per-set reactions (2026-08-18) — app v0.15.0, DEPLOYED (`16e7afb`).** Resolves the #3 open decision below — owner chose **full AI every set** (quota is a non-issue for one user: Gemini free tier ~1,500/day & 15/min vs ~27 sets/session). "Per-set reactions" mode now fires an **instant rule-based line on the tap** (so feedback never lags) and **replaces it with a specific AI reaction** when it lands — [`getReaction`](./src/features/coach/api.ts) → `coach` edge fn `kind:"reaction"`, whose prompt now demands progression coaching with the real numbers ("hit 7 → chase 8 / add load"), no generic filler. Non-blocking: a slow call or transient 503 just leaves the rule line; a `reactionToken` `useRef` in [`WorkoutLogger`](./src/features/workouts/WorkoutLogger.tsx) stops a late reaction from an earlier set clobbering a newer toast. Context sent: exercise, weight×reps, target range, last-time, RPE. **Depends on the v0.14.0 service-role key fix below** (AI confirmed firing). `tryGemini` now accepts `"reaction"`. Build green, `tsc` clean, `coach` redeployed.
...
> 4. **AI recap silently fell back to rule-based — ROOT-CAUSED & FIXED.** The **"Test coach AI"** button (new in Settings, `testCoachAI`) pinned it: *"Function ran but found no Gemini key on the server."* The key WAS saved (browser reads it), but every edge function read it with the **caller's forwarded JWT**, which didn't reliably reach the RLS'd `profiles` SELECT → the read came back empty → silent fallback. **Fix: authenticate the caller for their `user.id`, then read their own key with `SUPABASE_SERVICE_ROLE_KEY` (bypasses RLS, still scoped to their id).** Applied to **all four** functions (`coach`, `food-photo`, `food-text`, `nutrition-coach`) — all redeployed — because they shared the bug (so photo-macros / meal-describe / nutrition-coach were silently non-AI too). `tryGemini` also no longer swallows `data.error`; the functions now return a precise fallback reason. **Verified: Test coach AI returns ✓ a real Gemini reply.** ⚠️ **Recurring gotcha for future edge functions:** read a user's stored secret with the service role scoped to `user.id`, NOT the forwarded JWT.
...
>
> **#3 RESOLVED in v0.15.0 (above):** per-set feedback is now full AI every set with an instant rule-line fallback. `npm run build` green, `tsc` clean. Frontend v0.14.0 pushed (commits `c470bb8`); all 4 edge functions fixed + redeployed (`cb52f8d`, `91fe86f`). **No open work items from the second-workout round — all six + the AI root cause are shipped and verified.**

...
> **⚡ CURRENT STATUS (2026-08-16): Phase 1 COMPLETE + post-audit ROADMAP COMPLETE — app at v0.12.0, all live.** Phase 1 (M0–M7) plus all **14 competitive-audit improvements** ([`ROADMAP.md`](./ROADMAP.md): adaptive TDEE, rest timer, RPE, recents, fiber, stall/deload, coach cross-session memory, progress photos, tape measurements, dashboard nudges, data export) are built, deployed to Vercel, and verified. Edge functions live: `coach` (v4, sends session history), `food-photo`, `food-text`, `nutrition-coach`. `npm run build` green. **No open work items** — the audit backlog is closed. **Read [`HANDOFF.md`](./HANDOFF.md) for the exact current state + Runbook** (deploy, migrations, the PostgREST schema-cache gotcha, the empty-CLI-migration-history gotcha, type regen). The steps below describe the original cold-start build order and are kept for history.
>
...
**First-workout fixes (2026-08-14, app v0.6.0):** after the owner's first on-device session — (1) **the AI coach was removed from the dashboard entirely.** It had been calling the Gemini `coach` briefing on *every* dashboard load (uncached), which drained the ~250/day free-tier quota and was the true cause of the food-photo `429`s and the "coach gave generic answers" fallback. **AI now fires only on deliberate triggers** (post-workout recap + per-set reactions), never on passive screen loads — keep it that way. (2) The rule-based recap no longer congratulates an **empty session** (`buildRecap` early-returns for 0 working sets / 0 volume; `getRecap` skips the AI call too). (3) The bottom **nav bar** no longer uses `position: fixed` (it detached mid-scroll on iOS + `backdrop-filter`) — `AppShell` is now a full-height flex column with only `<main>` scrolling. (4) The **`Stepper`** accepts fine decimals (e.g. `0.6`) via a focused draft-string instead of a coerced controlled number. Full detail in [`HANDOFF.md`](./HANDOFF.md) (2026-08-14 note).
...
- **Supabase project:** `yotsunlngoudmxowiviq` · **Edge functions (all 4 deployed):** `coach` (briefing/recap/**reaction**), `food-photo`, `food-text`, `nutrition-coach`. All read the per-user Gemini key server-side **with the service role scoped to `user.id`** (see the Architecture note — the old forwarded-JWT read silently returned empty). **The CLI is installed locally** (linked to project ref `yotsunlngoudmxowiviq`) — deploy with `supabase functions deploy <name>` (or `npx supabase functions deploy <name>`; Docker not required, the "Docker is not running" line is a harmless warning), or paste the code in the Supabase dashboard → Edge Functions.
- **Free accounts needed:** Supabase, Google (OAuth + Gemini key), Vercel
- **Monthly cost (1 user):** $0

### Design system — "FitBody" (2026-08-18 restart; replaced "Calm Athletic")
- **One source of truth:** all colors, radii, and fonts are CSS tokens in [`src/index.css`](./src/index.css) `@theme` (dark-first) with `:root.light` overrides. Change the look there — components consume tokens via Tailwind utilities (`bg-brand`, `text-accent`) and the `.card` / `.card-2` / `.card-paper` classes. **Don't hardcode hex in components.** (The token *names* are semantic — `--color-brand`/`-accent`/`-surface` — so the 2026-08-18 restart just remapped their values; components didn't change.)
- **Identity (matches the FitBody Figma kit):** **violet brand** (`--color-brand #896CFE` = titles, primary buttons, nav, play), **lavender** (`--color-brand-soft #B3A0FF` = full-bleed `.section-band`s + nav fill), **lime accent** (`--color-accent #E2F163` = section headings, primary CTAs, badges — **always on black text** via `--color-on-accent`). Surfaces are **flat solid** (charcoal bg `#1E1E1E`, cards `#232323`; the old glass/blur is gone) plus a **white "paper" card** (`.card-paper`) for list rows. Signature moves: fully-rounded **pills** (buttons/chips/inputs), ~22px cards, two card variants (dark w/ lime title vs white w/ image-on-right + star badge), lavender section bands. Type is **League Spartan** everywhere (one geometric family — Bold headings via `.font-display`, Light body); loaded from Google Fonts in `index.html` (graceful system fallback). **Open point (deferred):** more imagery + "vivacity" (the kit leans on photos/vibrant blocks; the app is still comparatively sparse). See HANDOFF.md → the FitBody note for the full change list.
...
- **Imagery + gradients (2026-08-18):** real photos come from [`src/lib/images.ts`](./src/lib/images.ts) — **curated hotlinked Unsplash CDN** urls (zero disk), chosen category-accurately (`heroImage` by workout keyword, `mealImage` by meal type, `foodImage` deterministic per name). Always render them through [`Photo`](./src/components/Photo.tsx) (fade-in, optional scrim, **gradient fallback on error** — never a broken image). Gradients are utilities in `index.css` (`.bg-grad-brand/-energy/-hero`, `.scrim-bottom/-full`): **vibrant on heroes/CTAs/charts, subtle on cards.** Unsplash photos + Google Fonts are runtime-cached by the service worker (`vite.config.ts` workbox `runtimeCaching`, CacheFirst), so repeat loads are instant and work offline; a photo that still fails falls back to a gradient via `Photo`.
- **Never hang a loading spinner:** every screen `load()` wraps its data calls in [`withTimeout`](./src/lib/async.ts) (5 s) inside `Promise.allSettled`, **plus** a 6 s safety timer that clears `loading` unconditionally, **plus** a `try/finally`. A stalled request (flaky net or a cold/paused free-tier Supabase project) must never leave the spinner spinning — keep this pattern on new screens.
...
- **Routes, charts + scanner are code-split** — the Workouts/Nutrition/Body/Settings pages `React.lazy`-load per route (Dashboard + Login stay eager for a fast landing; `AppShell` wraps `<Outlet/>` in `<Suspense>`); `recharts` loads via `React.lazy` when a chart mounts; `@zxing/browser` via dynamic `import()` on first camera use. Keep new heavy pages behind lazy boundaries so the eager `index` chunk stays ~22 kB gzip.
...
- **Edge functions read a user's stored secret with the SERVICE ROLE, not the caller's JWT** (2026-08-18 fix, all 4 fns). Pattern: authenticate the caller with their token (`auth.getUser()`) to learn `user.id`, then read their row with a `SUPABASE_SERVICE_ROLE_KEY` client scoped to that id. Reading the RLS'd `profiles` row via the *forwarded* user JWT silently returned empty (the token didn't reach PostgREST) → every AI feature fell back to non-AI with no visible error. The service-role secret never leaves the server; scoping to `user.id` keeps it safe. Don't revert to a JWT-context read for per-user secrets. The `coach`/`food-*`/`nutrition-coach` fns also return a precise `error` string on fallback, and `Settings → Test coach AI` (`testCoachAI`) fires one call to surface it. AI degradation is still graceful everywhere (rule-based fallback), so a Gemini 429/503 or missing key never blocks the app.
